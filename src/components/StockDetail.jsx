...
// Math rationale for each "Why buy/sell/hold?" reason, per WHITEPAPER.md's
// Algorithms section -- gives the underlying indicator definition, not just
// the verdict.
function explainReason(reason) {
  if (reason.startsWith('RSI')) {
    return "Relative Strength Index (14, Wilder's smoothing) measures momentum on a 0-100 scale from average gains vs. losses. Above 55 reads as strength, below 45 as weakness; >70 / <30 flag overbought / oversold.";
  }
  if (reason.startsWith('MACD')) {
    return 'MACD (12/26/9): the gap between fast and slow EMAs. Histogram above zero is bullish momentum, below zero bearish.';
  }
  if (reason.endsWith('(MA)')) {
    return "Moving-average trend: the 50-period average vs. the 200-period (or longest available). 50 above 200 signals an uptrend, below signals a downtrend.";
  }
  return null;
}
...
export default function StockDetail({ stock, onClose, dark, t, onNavigate, currentIndex, totalCount }) {
  ...
  useEffect(() => {
    ...
    // Crosshair scrub -- filter Related News to the hovered period
    chart.subscribeCrosshairMove((param) => {
      if (!param || param.time == null) {
        setHoveredDate(null);
        return;
      }
      setHoveredDate(typeof param.time === 'number' ? new Date(param.time * 1000) : new Date(param.time));
    });
  ...
  const stats = useMemo(() => [
    ...
    { label: 'High', value: (d.high ?? stock?.dayHigh ?? stock?.high24h) ? formatCurrency(d.high ?? stock?.dayHigh ?? stock?.high24h) : '--' },
    { label: 'Low', value: (d.low ?? stock?.dayLow ?? stock?.low24h) ? formatCurrency(d.low ?? stock?.dayLow ?? stock?.low24h) : '--' },
    { label: 'Prev Close', value: (d.prevClose ?? stock?.prevClose) ? formatCurrency(d.prevClose ?? stock?.prevClose) : '--' },
    ...
    { label: 'Mkt Cap', value: formatMarketCap(d.marketCap ?? stock?.marketCap) },
  ...
  // Crosshair scrub: filter Related News to articles near the hovered date.
  // Intraday ranges (1m/15m/1d/5d) use a 1-day window; daily+ ranges use 3 days
  // so sparse news still surfaces a result.
  const displayedNews = useMemo(() => {
    if (!hoveredDate || news.length === 0) return news;
    const windowMs = (range === '1m' || range === '15m' || range === '1d' || range === '5d')
      ? 24 * 60 * 60 * 1000
      : 3 * 24 * 60 * 60 * 1000;
    const filtered = news.filter(a => {
      if (!a.publishedAt) return false;
      const t = new Date(a.publishedAt).getTime();
      return Number.isFinite(t) && Math.abs(t - hoveredDate.getTime()) <= windowMs;
    });
    return filtered.length > 0 ? filtered : news;
  }, [news, hoveredDate, range]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {tradeSignal && showSignalWhy && (
          <div style={{ ...glass, padding: '10px 14px', marginBottom: 10 }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: t.textSecondary, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>
              Why {tradeSignal.label.toLowerCase()}?
            </div>
            {(tradeSignal.reasons?.length ? tradeSignal.reasons : ['No strong technical signals either way — momentum and trend indicators are neutral.']).map((r, i) => {
              const explainer = tradeSignal.reasons?.length ? explainReason(r) : null;
              return (
                <div key={i} style={{ padding: '3px 0' }}>
                  <div style={{ fontSize: 12, color: t.text, display: 'flex', gap: 6 }}>
                    <span style={{ color: t.textTertiary }}>•</span><span>{r}</span>
                  </div>
                  {explainer && (
                    <div style={{ fontSize: 11, color: t.textTertiary, marginLeft: 16, marginTop: 2, lineHeight: 1.4 }}>
                      {explainer}
                    </div>
                  )}
                </div>
            ...
            <div style={{ fontSize: 10, color: t.textTertiary, marginTop: 6 }}>
              Technical analysis of recent price history. Not financial advice.
            </div>
          </div>
        )}
        ...
        <div ref={newsRef} style={{ marginTop: 4 }}>
          <div style={{
            ...
            {hoveredDate && displayedNews !== news && (
              <span style={{ textTransform: 'none', letterSpacing: 0, fontWeight: 500, color: t.textTertiary }}>
                near {hoveredDate.toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
              </span>
            )}
          </div>
          {newsLoading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {displayedNews.slice(0, 6).map((article, i) => {
                  ...
                  >
                    ...
                    <div style={{ padding: article.image ? '10px 12px 12px' : '12px' }}>
                      ...
                      <div style={{ fontSize: 10, color: t.textTertiary || t.textSecondary }}>
                        {article.publishedAt ? `${relativeTime(article.publishedAt)} · ${new Date(article.publishedAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}` : ''}
                      </div>
                    </div>
                  </a>
                );
