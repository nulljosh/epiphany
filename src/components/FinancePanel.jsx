...
const TAB_GROUPS = [
  { id: 'portfolio', label: 'Portfolio', tabs: ['portfolio', 'trade'] },
  { id: 'cashflow', label: 'Cash Flow', tabs: ['budget', 'spending', 'subscriptions'] },
  { id: 'outlook', label: 'Outlook', tabs: ['dashboard', 'roadmap'] },
];
const TAB_LABELS = {
  portfolio: 'Holdings', trade: 'Trade', budget: 'Budget', spending: 'Spending',
...
const INCOME_SCENARIOS = [
  ...
  { label: 'x2 Income', multiplier: 2, color: '#8CA0B3' },
];
      ...
      >
        <defs>
          ...
          <linearGradient id="savingsGrad" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={t.paleBlue} stopOpacity="0.32" />
            <stop offset="100%" stopColor={t.paleBlue} stopOpacity="0.06" />
          </linearGradient>
        ...
        {showSavings && savingsPoints.map((point, i) => {
          ...
          return (
            <g key={`savings-${i}`}>
              <circle cx={point.x} cy={point.y} r={3.5} fill={t.paleBlue} fillOpacity="0.9" />
              <text x={point.x} y={point.y - 8} textAnchor="middle" fill={t.paleBlue} fontSize="9" fontWeight="600" fontFamily="-apple-system, system-ui, sans-serif" opacity="0.85">
...
export default function FinancePanel({ dark, t, stocks, isAuthenticated }) {
  ...
  const editableIncome = Array.isArray(portfolioDraft?.budget?.income) ? portfolioDraft.budget.income : [];
  const editableExpenses = Array.isArray(portfolioDraft?.budget?.expenses) ? portfolioDraft.budget.expenses : [];
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 4, padding: '0 16px', marginBottom: 8, flexWrap: 'wrap' }}>
          ...
          return (
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                whiteSpace: 'nowrap',
                background: active ? t.text : t.glass,
            ...
            <Card dark={dark} t={t} style={{ marginBottom: 16 }}>
              <div style={sectionStyle}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  {brokerSnapshot?.linked && brokerSnapshot.syncedAt && (
                    <div style={{ fontSize: 10, color: t.textTertiary }}>
                      Synced {Math.max(0, Math.round((Date.now() - new Date(brokerSnapshot.syncedAt).getTime()) / 60000))}m ago
                    </div>
                  )}
                </div>
                {holdings.map((holding) => (
                  <div key={holding.symbol} style={rowStyle}>
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: t.textTertiary }}>
                        {holding.costBasis != null
                          ? <>{holding.shares.toFixed(4)} shares @ {formatCurrency(holding.costBasis)}</>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      {holding.gain == null ? (
                        <div style={{ fontSize: 11, color: t.textTertiary }}>Synced via SnapTrade</div>
                      ) : (
                ...
                {accounts.filter(a => a.balance > 0).map((account, index) => (
                  <div key={`${account.name}-${index}`} style={rowStyle}>
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: t.textTertiary }}>{capitalize(account.type)}</div>
                    </div>
                    <div style={{ fontWeight: 600, fontSize: 14, fontVariantNumeric: 'tabular-nums' }}>
            ...
            <Card dark={dark} t={t} style={{ marginBottom: 16, padding: 20 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: 12, marginBottom: 12 }}>
                  ...
                  {totalIncome > 0 && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      <div style={{ width: 8, height: 3, borderRadius: 2, background: t.paleBlue }} />
