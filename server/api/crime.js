...
async function fetchNewsBasedCrime(lat, lon) {
  ...
  // Reverse geocode to get city + broader region (county/state) + country for localized search
  const { city, region, countryCode } = await reverseGeocode(lat, lon);

  const searchTerms = [];
  if (city) searchTerms.push(city);
  if (region && region !== city) searchTerms.push(region);

  // Fetch crime news for the city and its broader region
  const fetchers = searchTerms.slice(0, 2).map(term => fetchCrimeNews(lat, lon, term, countryCode).catch(() => []));
...
// Reverse geocode to get city/region/country for localized news searches -- works worldwide
async function reverseGeocode(lat, lon) {
  try {
    ...
    return { city: null, region: null, countryCode: 'US' };
  }
}
...
async function fetchCrimeNews(lat, lon, cityName, countryCode = 'US') {
  ...
  try {
    const rss = await fetchText(
      `https://news.google.com/rss/search?q=${query}&hl=en&gl=${countryCode}&ceid=${countryCode}:en`
    );
...
async function handler(req, res) {
  ...
  try {
    ...
    // Query US/Canadian portals only if user is within 50km of that city
    for (const portal of [...US_PORTALS, ...CA_PORTALS]) {
    ...
    attemptedSources.push('news_fallback');
    fetchers.push(fetchNewsBasedCrime(lat, lon).catch(() => []));
    ...
    return res.status(200).json({
      incidents: [],
      attemptedSources: ['news_fallback'],
