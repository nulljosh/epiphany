...
export default async function handler(req, res) {
  ...
  // Places real orders with stored tokens — admin-only until per-user flow ships.
  const session = await getSessionUser(req);
  if (!session || !isAdmin(session.email)) return errorResponse(res, 401, 'Unauthorized');
