...
- [ ] SnapTrade brokerage connection error (five-day silent outage). Root cause diagnosed 2026-08-30: not a code bug. The `SNAPTRADE_CLIENT_ID` secret on the Cloudflare Worker was mangled when retyping secrets during the 2026-08-28 Vercel-to-Cloudflare cutover (Vercel returned secrets as [SENSITIVE]). SnapTrade dashboard showed no traffic for five days (last success Aug 25 at 7:14 PM). **Fixed 2026-08-30:** Set SNAPTRADE_CLIENT_ID to MAYBULB-LKHSV via `wrangler secret put`. Also refactored server/api/broker/sync.js to stop leaking raw SnapTrade errors in the response body (returns "Brokerage temporarily unavailable" instead, keeps detail in console.error). Tests pass. **Blocked on Joshua:** Tap "Connect brokerage" in the app to verify the fix. If it 401s with code 1076, the consumer key may also need rotating. The consumer key is masked on the SnapTrade dashboard (no reveal, only a Rotate button) so it could not be confirmed to match.
