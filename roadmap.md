...
- [ ] Remaining empty map layers, measured 2026-08-27 for downtown Vancouver: `/api/events` 502 (GDELT unavailable, no cache), `/api/flights` 400 (the map's own query shape is rejected), `traffic`/`emergency`/`weather-alerts`/`aqi`/`earthquakes` all 0. `crime` returning 0 is expected — it only covers 9 US cities, so it can never populate for Vancouver. Working: `news` (69 articles), `wildfires` (3).
...
- [ ] **BLOCKED ON JOSHUA — Epiphany iOS 2.5.5 is REJECTED and the reason needs a 2FA'd web login.** Verified 2026-08-27 via public API: version `6d3d9cb3-c183-4c9a-ac96-bfca52006486`, `appStoreState: REJECTED`, latest submission `1afd5ca2-4103-4da7-914f-fca3f2051915` in `UNRESOLVED_ISSUES` (submitted Aug 27 2026 01:56 PM PDT by API Key; `asc review history` shows outcome `rejected`, 1 item rejected). **The reason text lives only in Resolution Center — the public API never exposes it, and `asc web auth status` returns `{"authenticated": false}`, so `asc web review show` hangs waiting on Apple's 2FA prompt.** `ASC_WEB_2FA_CODE_COMMAND` has never been set on this machine, and `asc-login` deliberately drives asc's own interactive 2FA prompt, which needs a code off a trusted device. **Exact manual step: run `asc-login`, enter the 2FA code, then `asc web review show --app 6779522175 --submission 1afd5ca2-4103-4da7-914f-fca3f2051915 --plain-text`.**
      - Do NOT resubmit blind. Both builds are VALID and attachable the moment the reason is known: `202608271349` (the intended 2.5.5 build) and `202608271355` (the 2.5.6 upload made by mistake).
      - Prior cycle for context: the reviewer screenshots cached at `.asc/web-review/6779522175/84f6bcd6-8160-4e7b-9a96-e9c53e959110/` show **Sign in with Apple failing with "Sign-Up Not Completed"** on iPad (Aug 26) — the 2.1(a) "unable to authenticate" rejection that 2.5.5 was built to fix.
      - Audited that fix this pass and it is **not obviously the cause of the new rejection**: `server/api/_apple-jwt.js` verifies RS256 against Apple's JWKS, pins issuer `https://appleid.apple.com`, checks `exp`/`iat` and requires `sub`; `APPLE_AUDIENCES` defaults to `com.heyitsmejosh.epiphany`, which matches `PRODUCT_BUNDLE_IDENTIFIER` in `ios/project.yml:55`, so native tokens validate. Probed production directly: `POST /api/auth?action=signin-apple` returns 401 `Invalid Apple token` for a forged token and 400 for a missing one — the verification path is live and behaving. So the new rejection is most likely a *different* issue, which is exactly why the reason must be read rather than guessed.
- [x] **Answered 2026-08-27 — 2.5.5 never reached the App Store, and there is no 2.6.x App Store record at all.** `asc versions list --app 6779522175` returns 7 version records; the highest iOS version in `READY_FOR_SALE` is **2.5.4** (that is what the public sees), with 2.5.3 / 2.5.2 / 2.5.1 / 1.0 behind it and macOS 2.5.2 live. iOS **2.5.5 is REJECTED**, so TestFlight build `202608271349` is beta-only. No 2.6.0/2.6.1 version record exists on the App Store — the "v2.6.1" in this repo's CLAUDE.md is the web/product version, not a shipped iOS release. Note: the repo table in `~/Documents/Code/CLAUDE.md` still claims epiphany is "v2.6.1+ live + App Store", which is stale (its own verified Ship Status block lower down correctly says iOS 2.5.4).
