...
## Unverified 2026-08-04
- [ ] **Is epiphany actually deploying on push?** `.vercel/project.json` is missing locally (the dir exists but is empty) — the exact drift bug just fixed in nimble, and the same class as Lexly's Pages project not being git-connected, which silently served 2-week-old content and cost a probable App Store rejection. The Vercel Blob statement change (`0fec55c`) is **server-side** and does nothing until deployed, so if push-to-deploy is not wired, statement upload is still broken in production regardless of tonight's fix. Verify: confirm the live deployment includes `0fec55c`, and either re-link the project (`vercel link`) committing `project.json` like nimble now does, or add an explicit deploy script. Until confirmed, treat the statement fix as NOT live.
