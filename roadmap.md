...
- [x] **REJECTED**: iOS 2.5.5, Guideline 2.1(a), reviewed Aug 26 2026 on iPad Air 11-inch (M3) / iPadOS 26.6.1. Apple reported both "Sign in with Apple" and "log in with provided credentials" showing an error. Investigated 2026-08-27: **could not reproduce, both paths work now.** Email/password verified against production with the real demo account (HTTP 200, tier=pro) and verified end to end on an iPad Air 11-inch simulator from a clean install (`isLoggedIn=true error=none`). Backend register+login also verified 201/200. Best explanation is the Workers abandoned-response-body cascade that was active on the review date (see the /api/stocks 500 entry) — it killed in-flight responses across the Worker, which would fail *both* auth paths at once exactly as reported. Fixed alongside: Apple token verification (below) now returns a clean 401 instead of a 500, and the dead Google/Facebook login buttons were removed (independent 2.1 risk). Ready to resubmit; watch the review on a real iPad if it comes back.
- [x] **SECURITY**: `/api/auth?action=signin-apple` accepted an unverified Apple identity token — it base64-decoded the payload without checking the signature, issuer, audience or expiry, and preferred the caller-supplied email over Apple's claim when linking accounts. A hand-written unsigned JWT naming any email returned a valid session for that account; confirmed against production, then fixed and re-verified (now 401). Tokens are verified RS256 against Apple's JWKS via WebCrypto (`server/api/_apple-jwt.js`, 8 tests).
