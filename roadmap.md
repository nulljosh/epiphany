...
- [x] Mobile web: on login the entire screen is instantly covered by the situation monitor. Fixed 2026-08-25 in `c8b4db2` (mobile sheet used to init open on <=768px); verified still closed-by-default 2026-08-28. Also fixed the breakpoint mismatch that made it fire at exactly 768px (iPad portrait): JS probed `max-width: 768px` while the CSS desktop grid starts at `min-width: 768px`, so both matched and the mobile sheet + fixed backdrop laid over the desktop layout. `src/App.jsx` now probes 767.98px.
...
- [x] `epiphany/CLAUDE.md`'s "Free vs Premium" table — corrected 2026-08-28 against `server/api/gates.js` and every `isPro` call site. Portfolio/watchlist/ontology writes are confirmed ungated; gated set is autopilot + `broker/trade`, Daily Brief, People graph. Added the key caveat the table was missing: `isProByEmail` returns true for every signed-in user unless `EPIPHANY_REQUIRE_PRO=true` (unset), so nothing is actually paywalled in production. Also retired the stale "autopilot is the only gate / line 330-335" cross-reference higher up in CLAUDE.md.
