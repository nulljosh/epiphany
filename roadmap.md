...
- [x] SnapTrade brokerage connection error (five-day silent outage). Root cause diagnosed 2026-08-30: not a code bug. The `SNAPTRADE_CLIENT_ID` and `SNAPTRADE_CONSUMER_KEY` secrets on the Cloudflare Worker were mangled when retyping secrets during the 2026-08-28 Vercel-to-Cloudflare cutover (Vercel returned secrets as [SENSITIVE]). SnapTrade dashboard showed no traffic for five days (last success Aug 25 at 7:14 PM). **Fully fixed 2026-09-02:** Rotated `SNAPTRADE_CONSUMER_KEY` on the SnapTrade dashboard and set both secrets on the Worker via `wrangler secret put`. Refactored server/api/broker/sync.js to stop leaking raw SnapTrade errors in the response body (returns "Brokerage temporarily unavailable" instead, keeps detail in console.error). Added self-healing when SnapTrade orphans a user: delete and re-register on next sync (code 1010). Also pulled transaction history to compute cost basis for holdings and trailing 12-month dividend income. Sync is live. Joshua must re-link Wealthsimple once.
...
- [x] 2026-09-02: SnapTrade consumer key rotated + set on the Worker; sync self-heals orphaned SnapTrade users (code 1010). Joshua must re-link Wealthsimple once.
