...
- [ ] SnapTrade brokerage connection error persists even though the $1 bill was paid this month. Diagnosed 2026-08-30: not a code bug. SnapTrade returns 401 code 1083 "Invalid clientId" at `registerUser`, so the `SNAPTRADE_CLIENT_ID` secret on the Cloudflare Worker is not a clientId SnapTrade recognises — it was re-typed by hand during the 2026-08-28 Workers cutover because Vercel would not decrypt it. Suspect the Maybulb->Jaybulb org rename retired the recorded `MAYBULB-LKHSV` key. **Blocked on Joshua:** sign in at dashboard.snaptrade.com, then re-set both secrets with `printf %s '<value>' | npx wrangler secret put ...` (never `echo` — a trailing newline reproduces this same 1083) and `npm run deploy`.
