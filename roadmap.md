...
- [ ] **Still broken as of 2026-08-04** per user report, *after* all fixes above — either a regression, an uncovered path, or server-side in parse/upload. Needs a repro with a real statement PDF before further edits.
- [ ] **Unverified: is epiphany actually deploying on push?** `.vercel/project.json` is missing locally (same drift class that silently served nimble/lexly stale content). The Blob migration (`0fec55c`) is server-side and does nothing until deployed — if push-to-deploy isn't wired, statement upload is still broken in production regardless of the fixes above. Verify the live deployment includes `0fec55c`; re-link (`vercel link`, commit `project.json`) or add an explicit deploy script if not. **Treat the statement fix as NOT confirmed live until this is checked** — likely the actual cause of the "still broken" report above, check this first.
