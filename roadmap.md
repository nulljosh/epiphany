...
- [ ] **SnapTrade production key billing failed — deadline 2026-08-06 (today).** SnapTrade emailed 2026-08-03: payment for July 31 did not process (org Maybulb, key MAYBULB-LKHSV, $1.00 USD due). If unpaid, the key is disabled, killing all live holdings/account sync. Pay/update billing via the SnapTrade dashboard link in the email (billing@snaptrade.com). **Check current status — deadline may have already passed.** (2026-08-25 probed: not determinable from the CLI — no working SnapTrade creds on disk, `.env.tui.local` has the vars empty, and `vercel env ls` returns nothing for this repo. api.snaptrade.com itself is up (200). Confirming whether the key is disabled requires the SnapTrade dashboard login, so this is **blocked on Joshua** — not an unknown anyone here can close.)
