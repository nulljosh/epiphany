...
- [ ] `epiphany/CLAUDE.md`'s "Free vs Premium" table is stale — it lists Portfolio/watchlist and Ontology writes as Premium; neither is gated in code. Fix before it is ever quoted to App Review.
...
## Stashed 2026-08-28
- [ ] **The Guideline 3.1.1 fix is committed but NOT LIVE.** `server/api/gates.js` (commit `7b50085`, 2026-08-28 01:30) grants Pro to any signed-in user unless `EPIPHANY_REQUIRE_PRO=true`, which is what makes the truthful answer to Apple's 2.1(b) question 4 ("none") actually true. Production `epiphany.heyitsmejosh.com` still serves a deployment created 2026-08-26 (Ready), so until this deploys, that answer is false — do not send the 2.1(b) reply in `notes/2-1-b-business-model-reply.md` before confirming the deploy.
  Deploy history tonight: four `vercel --prod` attempts. The first printed `Error: fetch failed` while exiting 0 (a false success — never trust the exit code here). Later attempts created deployments `epiphany-chljt29jx` and `epiphany-e6cr9pkms` that sat in `Building…` for 20+ minutes without promoting; `vercel inspect` reported their status as UNKNOWN throughout. Build log: `vercel.com/nulljosh-9577s-projects/epiphany/CYKpmtnCqeNuiUQx6YuPJBG1M9Vd`.
  To verify once resolved: `npx vercel inspect https://epiphany.heyitsmejosh.com` and confirm `created` is newer than the gates.js commit. Do NOT verify by curling a gated endpoint unauthenticated — `/api/daily-brief` and `/api/people` return 402 under both the old and new code, so that probe proves nothing.
