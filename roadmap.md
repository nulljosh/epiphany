...
## Urgent
- [x] **Portfolio balance mismatch — no longer reproducing (2026-08-25).** Was: screen showed $75.35 total against $193.93 + MSB $75.25 = $269.18 in Holdings. Joshua reports it resolved on its own, and SnapTrade billing turned out to be paid the whole time, so the likely cause was a transient sync/stale-holdings state rather than a totals bug. Closing as not-reproducing, not as fixed — if the totals disagree again, suspect duplicate reconnected accounts first (see the stale-holdings note).
...
- [x] **SnapTrade production key billing — PAID.** Joshua confirmed 2026-08-25 that the outstanding $1.00 USD on key MAYBULB-LKHSV was paid, so the key was never disabled and live holdings/account sync is intact. Closed.
