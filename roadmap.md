...
- [x] Epiphany Mac 1.0 PREPARE_FOR_SUBMISSION — RESOLVED, verified 2026-07-26: this predates the Mac→iOS app-record merge (see "ASC merge" item above). `asc versions list --app 6779522175 --platform MAC_OS` shows 2.5.2 `READY_FOR_SALE`. Nothing left to do here.

## From Epiphany.pdf (imported 2026-07-19)
- [x] News drawer slide up/down animation is choppy/glitchy — FIXED 2026-07-26: root cause was the bouncy overshoot easing (`cubic-bezier(0.34, 1.56, 0.64, 1)`) animating a `backdrop-filter: blur(20px)` element — expensive to recomposite every frame, especially through the overshoot/settle phase. Swapped to `ease-out` (no overshoot) + `willChange: transform` in `src/components/MarketsPanel.jsx`, no transform-offset or hit-testing changes (previous fix attempt #3 broke drag hit-testing this way — avoided that path entirely).
...
- [x] "What's New" modal too big — AUDITED 2026-07-26, not a code bug: web (`src/components/WhatsNew.jsx`) is already capped `maxWidth: 360`; iOS (`ios/WhatsNewSheet.swift`) already sizes itself to content via `GeometryReader` + `.presentationDetents([.height(sheetHeight)])`. If it still looks big on-device, it's a stale TestFlight/simulator build, not the current code.
- [x] Screenshots need refreshing (App Store) — same predates-the-Mac-merge item as PREPARE_FOR_SUBMISSION above; Mac record is `READY_FOR_SALE` at 2.5.2. If iOS screenshots specifically need a refresh, that's a separate, not-yet-filed ask — none found blocking anything right now.
