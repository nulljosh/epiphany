...
## Urgent
- [ ] **BLOCKING 2026-08-28: Epiphany 2.5.6 build attached, pending submit — four stale draft reviewSubmissions blocking concurrency (max 5).**
  Build 202608280938 uploaded VALID, version retitled in place from 2.5.5 rejection, encryption declaration set, `asc validate` passes (0 errors/0 blocking). Final `asc review submit` returns "Version is not ready to be submitted yet, please try again later" — Apple-side state settling after attaching a build to a previously-rejected version. A background retry loop is running. Four empty READY_FOR_REVIEW draft submissions (no `submittedDate`, zero items) sit in the app's submission queue, hitting Apple's "maximum limit=5 of concurrency" and blocking new submissions. These draft submissions **cannot be cancelled via CLI** ("Resource is not in cancellable state") — dashboard-only cleanup via the blue Draft Submissions panel required. Chrome extension not connected this session, so dashboard automation unavailable; cleanup must be manual. See [[reference_asc_stray_curvely_submission]] for the full trap.
