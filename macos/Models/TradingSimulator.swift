...
final class TradingSimulator {
    ...
    private func findEntryIfNeeded() {
        ...
        for symbol in assetBasePrices.keys.sorted() {
            ...
            let avg10 = average(of: Array(series.suffix(10)))
            let avg20 = average(of: Array(series.suffix(20)))
            ...
            let prior10 = Array(series.dropLast().suffix(10))
            ...
            let volatility = stddev(of: Array(series.suffix(10))) / avg10
            guard volatility < 0.025 else { continue }

            let recent = Array(series.suffix(10))
            let risingBars = zip(recent.dropLast(), recent.dropFirst()).filter { $1 > $0 }.count
