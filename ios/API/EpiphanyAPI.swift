...
final class EpiphanyAPI: @unchecked Sendable {
    ...
    func fetchSparklineData(symbol: String) async throws -> [Double] {
        let cacheKey = "sparkline:\(symbol)"
        if let cached: [Double] = cached(cacheKey) {
            return cached
        }
        ...
        let request = URLRequest(url: url)
        let data = try await perform(request)
        let history = try decode(PriceHistory.self, from: data)
        let prices = history.history.compactMap { $0.close }
    ...
    func fetchAutopilot() async throws -> AutopilotState {
        let url = try makeURL("/api/broker/autopilot")
        let data = try await perform(URLRequest(url: url))
        return try decode(AutopilotState.self, from: data)
    }

    @discardableResult
    func updateAutopilot(_ settings: AutopilotSettings) async throws -> AutopilotSettings {
        let url = try makeURL("/api/broker/autopilot")
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        ...
        return try decode(Response.self, from: data).settings
    }

    func placeManualTrade(accountId: String, symbol: String, side: String, qty: Double) async throws {
        let url = try makeURL("/api/broker/trade")
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        let body = [
