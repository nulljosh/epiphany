...
struct PortfolioView: View {
                ...
                if !appState.isLoggedIn {
                    ...
                    ScrollView {
                        VStack(spacing: 12) {
                            ...
                            if let accounts = appState.financeData?.accounts {
                                accountsBreakdown(accounts)
                            }
    ...
    private func accountsBreakdown(_ accounts: [FinanceData.Account]) -> some View {
        let nonEmpty = accounts.filter { $0.balance >= 1.0 }
        guard !nonEmpty.isEmpty else { return AnyView(EmptyView()) }
        return AnyView(sectionCard("Accounts") {
            VStack(spacing: 12) {
                ForEach(nonEmpty) { account in
                    HStack(alignment: .top) {
                        ...
                        Spacer(minLength: 12)
                        Text(CurrencyFormatter.formatPrice(account.balance))
                            .font(.callout.weight(.semibold))
                            .foregroundStyle(Palette.text)
                    }
    ...
    private var statementsChart: some View {
        ...
        let actuals = grouped
            .map { (month: $0.key, total: $0.value.filter { $0.amount < 0 }.reduce(0) { $0 + abs($1.amount) }, transactions: $0.value, predicted: false) }
