...
struct StockDetailView: View {
    ...
    let stock: Stock
    // Paged container (StockDetailPageView) renders one shared star; standalone
    // pushes keep their own. Two pages rendering toolbars mid-swipe = two stars.
    var showsWatchlistToolbar = true
    ...
    @State private var tradeSide = "buy"
    @State private var tradeQty = ""
    @State private var tradeLoading = false
    ...
    var body: some View {
            ...
            NavigationStack {
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: 12) {
                        ForEach(relatedNews) { article in
        ...
        .sheet(isPresented: $showTradeSheet) {
            VStack(alignment: .leading, spacing: 12) {
                HStack {
                    ...
                    Spacer()
                    Button(action: { showTradeSheet = false }) {
                ...
                Button(action: {
                    guard let qty = Double(tradeQty), qty > 0 else {
                        tradeError = "Invalid quantity"
                        return
                    }
                    Task {
                        tradeLoading = true
                        tradeError = nil
                        do {
                            let accountId = appState.financeData?.accounts.first?.id ?? ""
                    ...
                    HStack {
                        Spacer()
                        Text(tradeLoading ? "..." : (tradeSide == "buy" ? "Buy" : "Sell"))
                            .font(.callout.weight(.semibold))
                        Spacer()
                    }
                    .foregroundStyle(.white)
                    .padding(.vertical, 10)
                    .background(tradeSide == "buy" ? .green : .red, in: RoundedRectangle(cornerRadius: 8))
                }
                .disabled(tradeLoading || tradeQty.isEmpty)
        ...
        .toolbar {
            if showsWatchlistToolbar && appState.isLoggedIn {
                ToolbarItem(placement: .topBarTrailing) {
                    WatchlistStarButton(symbol: stock.symbol)
                }
            }
        }
    ...
    // ponytail: pulled out of a ternary in chartView; the mixed flatMap/keypath
    // expression blew the type-checker budget under Xcode 27. Control flow can't
    // live in the @ViewBuilder body, so it lives here.
    private static func priceRangeValues(usesOHLC: Bool, candles: [OHLCPoint], points: [(Date, Double)]) -> [Double] {
        if usesOHLC {
            var out: [Double] = []
    ...
    // ponytail: the 140-line Chart builder blew the type-checker budget under
    // Xcode 27. Split in two so each body is checked on its own.
    @ChartContentBuilder
    ...
    @ChartContentBuilder
    private func overlayMarks() -> some ChartContent {
            // Indicators
    ...
    private var chartView: some View {
        ...
        let allPrices = Self.priceRangeValues(usesOHLC: usesOHLC, candles: candleData, points: points)
    ...
    var body: some View {
        TabView(selection: $currentIndex) {
            ForEach(Array(stocks.enumerated()), id: \.element.id) { index, stock in
                StockDetailView(stock: stock, showsWatchlistToolbar: false)
                    .tag(index)
            }
        }
        ...
        .navigationBarTitleDisplayMode(.inline)
        .toolbar {
            if appState.isLoggedIn, stocks.indices.contains(currentIndex) {
                ToolbarItem(placement: .topBarTrailing) {
                    WatchlistStarButton(symbol: stocks[currentIndex].symbol)
                }
            }
        }
    }
...
struct WatchlistStarButton: View {
    ...
    var body: some View {
        Button {
            Task {
                if isWatchlisted {
                    await appState.removeWatchlistSymbol(symbol)
                } else {
                    await appState.addWatchlistSymbol(symbol)
                }
            }
        } label: {
