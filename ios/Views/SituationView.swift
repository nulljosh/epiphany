...
struct SituationView: View {
            ...
            HStack(spacing: 6) {
                ForEach(VenueCategory.allCases, id: \.self) { cat in
                    Button {
                        Haptics.impact(.light)
                        if selectedVenueCategories.contains(cat) {
                            selectedVenueCategories.remove(cat)
                            venueResults[cat] = nil
                        } else {
                            selectedVenueCategories.insert(cat)
    ...
    private func searchVenues(_ category: VenueCategory) async {
        ...
        // Search the visible map region, not the GPS region -- panning to a new
        // city must surface that city's venues, not home's.
        request.region = visibleRegion ?? currentRegion
        request.pointOfInterestFilter = MKPointOfInterestFilter(including: [category.poiCategory])
        request.resultTypes = .pointOfInterest
        if let response = try? await MKLocalSearch(request: request).start() {
            venueResults[category] = response.mapItems
        }
    }

    private func refreshVenues() async {
        for cat in VenueCategory.allCases where selectedVenueCategories.contains(cat) {
            await searchVenues(cat)
        }
    }
